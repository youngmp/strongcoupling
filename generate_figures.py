...
def rhs_cgl(t,y,obj,eps):
    ...
    y_reverse = np.zeros(4)
    y_reverse[:2] = y[2:]
    y_reverse[2:] = y[:2]
    z[:2] = obj.rhs(t,y[:2]) + eps*obj.coupling(y)
    z[2:] = obj.rhs(t,y[2:]) + eps*obj.coupling(y_reverse)
    #(z,t)
    
    return z


def cgl_diffs(ax,a,eps,recompute=True,Tf=0):
    """
    show numerical simulation of phase differences over time
    """
    #T = 10.648268787326938
    
    # simulate series of cgl models for different initial conditions
    ...
    #sim_time = np.linspace(0,Tf,100000)
    theta_vals = np.arange(T/200,T,T/200)
    
    for i,theta in enumerate(theta_vals):
        #print(theta)
        
        fname = (a.dir+'cgl_diff_phi='+str(theta)
                 +'_eps='+str(eps)+'_Tf='+str(Tf))+'.txt'
        ...
        if file_does_not_exist or recompute:
            ...
            init = [LC[phase1_idx,1:][0],LC[phase1_idx,1:][1],
                    LC[phase2_idx,1:][0],LC[phase2_idx,1:][1]]
            
            
            #print(init)
            sol = solve_ivp(rhs_cgl,[0,Tf],init,args=(a,eps),method='LSODA',
                            atol=1e-8,rtol=1e-8)
            
            
            if False:
                print(LC[phase1_idx,0],init[0])
                fig = plt.figure()
                ax = fig.add_subplot(111)
                ax.plot(sol.t,sol.y.T[:,0])
                ax.plot(sol.t,sol.y.T[:,3])
                #ax.scatter(LC[phase1_idx,0],init[0])
                #ax.scatter(LC[phase1_idx,0],init[1])
                
                plt.show(block=True)
            
            v1_peak_idxs = find_peaks(sol.y[0,:])[0]
            v2_peak_idxs = find_peaks(sol.y[2,:])[0]
            #print(v2_peak_idxs)
...
def cgl_h_deprecated():
    ...
    parlabel = [r' $\varepsilon = '+str(epslist[0]) + '$',
                r' $\varepsilon = '+str(epslist[1]) + '$']
    order_text = ['Order 2', 'Order 4', 'Order 10']
    ...
    for i,eps in enumerate(epslist):
        options['d_val'] = dlist[i]
        a = CGL(**options)
        
        a.load_h()
        
        phi = np.linspace(0,a.T,a.NB)
        
        start = time.time()
        axs[1,i] = cgl_diffs(axs[1,i],a,eps,Tf=tlist[i])
        end = time.time()
        ...
        for j in range(len(termlist)):
            ...
            axs[0,i].plot(phi,h,color=col[j])
    ...
    axs[0,0].legend(custom_lines,order_text,
